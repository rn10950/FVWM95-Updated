...
MainFrame::MainFrame(const OXWindow *p, int w, int h) :
  ...
  AddFrame(hf, new OLayoutHints(LHINTS_EXPAND_ALL));

  //------ GL frame

  OXCompositeFrame *glf1 = new OXCompositeFrame(hf, 10, 10,
                                                SUNKEN_FRAME | DOUBLE_BORDER);
  hf->AddFrame(glf1, new OLayoutHints(LHINTS_EXPAND_ALL));

  _glView1 = new GLView(glf1);
  glf1->AddFrame(_glView1, new OLayoutHints(LHINTS_EXPAND_ALL));

  OXCompositeFrame *glf2 = new OXCompositeFrame(hf, 10, 10,
                                                SUNKEN_FRAME | DOUBLE_BORDER);
  hf->AddFrame(glf2, new OLayoutHints(LHINTS_EXPAND_ALL));

  _glView2 = new GLView(glf2);
  glf2->AddFrame(_glView2, new OLayoutHints(LHINTS_EXPAND_ALL));
