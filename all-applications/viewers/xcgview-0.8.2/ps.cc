...
struct document *psscan(FILE *file)
{
    struct document *doc;
    int bb_set = PS_NONE;
    int pages_set = PS_NONE;
    int page_order_set = PS_NONE;
    int orientation_set = PS_NONE;
    int page_bb_set = PS_NONE;
    int page_media_set = PS_NONE;
    ...
    char *label;
    char line[PS_LINELENGTH];	/* 255 characters + 1 newline + 1 NULL */
    char text[PS_LINELENGTH];	/* Temporary storage for text */
	...
	} else if (bb_set == PS_NONE && iscomment(line+2, "BoundingBox:")) {
	...
	} else if (orientation_set == PS_NONE &&
		   iscomment(line+2, "Orientation:")) {
	    sscanf(line+length("%%Orientation:"), "%s", text);
	    if (strcmp(text, "(atend)") == 0) {
		orientation_set = PS_ATEND;
	    } else if (strcmp(text, "Portrait") == 0) {
		doc->orientation = PS_PORTRAIT;
		orientation_set = 1;
	    } else if (strcmp(text, "Landscape") == 0) {
		doc->orientation = PS_LANDSCAPE;
		orientation_set = 1;
	    }
	} else if (page_order_set == PS_NONE && iscomment(line+2, "PageOrder:")) {
	    sscanf(line+length("%%PageOrder:"), "%s", text);
	    if (strcmp(text, "(atend)") == 0) {
		page_order_set = PS_ATEND;
	    } else if (strcmp(text, "Ascend") == 0) {
		doc->pageorder = PS_ASCEND;
		page_order_set = 1;
	    } else if (strcmp(text, "Descend") == 0) {
		doc->pageorder = PS_DESCEND;
		page_order_set = 1;
	    } else if (strcmp(text, "Special") == 0) {
		doc->pageorder = PS_SPECIAL;
		page_order_set = 1;
	    }
	} else if (pages_set == PS_NONE && iscomment(line+2, "Pages:")) {
	...
	while (readline(line, sizeof line, file, &position, &line_len) &&
	    ...
	    if (!DSCcomment(line)) {
		/* Do nothing */
	    } else if (doc->default_page_orientation == PS_NONE &&
		iscomment(line+2, "PageOrientation:")) {
		sscanf(line+length("%%PageOrientation:"), "%s", text);
		if (strcmp(text, "Portrait") == 0) {
		    doc->default_page_orientation = PS_PORTRAIT;
		} else if (strcmp(text, "Landscape") == 0) {
		    doc->default_page_orientation = PS_LANDSCAPE;
		}
	    } else if (page_media_set == PS_NONE &&
...
continuepage:
	while (readline(line, sizeof line, file, &position, &line_len) &&
	    ...
	    if (!DSCcomment(line)) {
		/* Do nothing */
	    } else if (doc->pages[doc->numpages].orientation == PS_NONE &&
		iscomment(line+2, "PageOrientation:")) {
		sscanf(line+length("%%PageOrientation:"), "%s", text);
		if (strcmp(text, "Portrait") == 0) {
		    doc->pages[doc->numpages].orientation = PS_PORTRAIT;
		} else if (strcmp(text, "Landscape") == 0) {
		    doc->pages[doc->numpages].orientation = PS_LANDSCAPE;
		}
		...
		free(cp);
	    } else if ((page_bb_set == PS_NONE || page_bb_set == PS_ATEND) &&
		       iscomment(line+2, "PageBoundingBox:")) {
		sscanf(line+length("%%PageBoundingBox:"), "%s", text);
		if (strcmp(text, "(atend)") == 0) {
		    page_bb_set = PS_ATEND;
		} else {
		    if (sscanf(line+length("%%PageBoundingBox:"), "%d %d %d %d",
			...
			if (page_bb_set == PS_NONE) page_bb_set = 1;
		    else {
			float fllx, flly, furx, fury;
	...
	} else if (bb_set == PS_ATEND && iscomment(line+2, "BoundingBox:")) {
	    if (sscanf(line+length("%%BoundingBox:"), "%d %d %d %d",
		       &(doc->boundingbox[LLX]),
		       &(doc->boundingbox[LLY]),
		       &(doc->boundingbox[URX]),
	...
	} else if (orientation_set == PS_ATEND &&
		   iscomment(line+2, "Orientation:")) {
	    sscanf(line+length("%%Orientation:"), "%s", text);
	    if (strcmp(text, "Portrait") == 0) {
		doc->orientation = PS_PORTRAIT;
	...
	} else if (page_order_set == PS_ATEND && iscomment(line+2, "PageOrder:")) {
	    sscanf(line+length("%%PageOrder:"), "%s", text);
	...
	} else if (pages_set == PS_ATEND && iscomment(line+2, "Pages:")) {
	    if (sscanf(line+length("%%Pages:"), "%*u %d", &i) == 1) {
		if (page_order_set == PS_NONE) {
		    if (i == -1) doc->pageorder = PS_DESCEND;
		    else if (i == 0) doc->pageorder = PS_SPECIAL;
		    else if (i == 1) doc->pageorder = PS_ASCEND;
		}
	    }
	}
    }
...
{
    char text[PS_LINELENGTH];	/* Temporary storage for text */
    char save[PS_LINELENGTH];	/* Temporary storage for text */
    char *cp;
    unsigned int num;
    unsigned int nbytes;
    int i;
...
char *pscopyuntil(FILE *from, FILE *to,
                 long begin, long end, const char *comment)
{
    char line[PS_LINELENGTH];	/* 255 characters + 1 newline + 1 NULL */
    char text[PS_LINELENGTH];	/* Temporary storage for text */
    unsigned int num;
