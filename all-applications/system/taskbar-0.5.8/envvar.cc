/**************************************************************************
 *
 *  Routines to expand environment-variables into strings.
...
static void strIns(char *s, const char *ins, int idx, int maxstrlen) {
  ...
  while (move-- > 0) *p2-- = *p1--;
  p1 = s + idx;
  if (idx + li >= maxstrlen) li = maxstrlen - idx - 1;
  while (li--) *p1++ = *ins++;
  s[maxstrlen - 1] = '\0';
}
...
static const char *getEnv(const char *name) {
  ...
  if ((tmp = StrDup(name)) == NULL) return empty; // better than no test at all.
  p = tmp;
  if (*p == '$') ++p;
  if (*p == '{') {
    ++p;
  ...
  if ((ret = getenv(p)) == NULL) ret = empty;
  delete[] tmp;
...
//                variables expanded. Use delete[] to deallocate the
...
char *envDupExpand(const char *s, int extra) {
  ...
  if (bufflen < slen + 1) bufflen = slen + 1;

  ret = new char[bufflen];
